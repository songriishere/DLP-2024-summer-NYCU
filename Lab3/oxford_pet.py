...
def load_dataset(data_path, mode):
    ...
    def training_data_preprocessing(image , mask , trimap):
        if mode == "train":
            degree = np.random.randint(-15, 16) #角度從-15~15
            ...
            horizontal = np.random.randint(2)
            if horizontal == 1:
    ...
    return SimpleOxfordPetDataset(root=data_path , mode = mode , transform = training_data_preprocessing)


if __name__ == "__main__":
    print("Current Working Directory: ", os.getcwd())
    # extract_archive('dataset/images.tar.gz')
    # extract_archive('dataset/annotations.tar.gz')
    Data = OxfordPetDataset(root="dataset", mode="valid")
