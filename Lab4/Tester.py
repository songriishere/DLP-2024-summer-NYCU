...
class Test_model(VAE_Model):
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        for index in range(1, 630):
            img_out = self.frame_transformation(decoded_frame_list[index-1].to(self.args.device))
            label_out = self.label_transformation(label[index]).to(self.args.device)
            z, mu, logvar = self.Gaussian_Predictor(img_out, label_out)

            z_like = torch.randn_like(z)
            decoder_out = self.Decoder_Fusion(img_out, label_out, z_like)
            decoder_out = self.Generator(decoder_out)
            decoded_frame_list.append(decoder_out.cpu())
            label_list.append(label_out.cpu())
